...
if ($category>0){  
    ...
    WHERE appointment_status ='$category' ORDER BY FIELD(appointment_status, 1, 2, 3, 4, 6, 5 ), Reg_Date DESC ";
    }
else
{ 
     $query_recpat="SELECT * FROM app_form as A 
...
if ($category>0){  
   ...
   $query_recpatuser = "SELECT * FROM app_form as A 
    left join login as B on(A.loginID = B.login_id) 
    left join patient_tbl as C on(A.pat_id=C.pat_id)
    WHERE login_id='$id' AND appointment_status ='$category' ORDER BY FIELD(appointment_status, 1, 2, 3, 4, 6, 5 ), Reg_Date DESC ";
    }
else
{ 
     $query_recpatuser="SELECT * FROM app_form as A 
    left join login as B on(A.loginID = B.login_id) 
    left join patient_tbl as C on(A.pat_id=C.pat_id)
    WHERE login_id='$id' ORDER BY FIELD(appointment_status, 1, 2, 3, 4, 6, 5 ), Reg_Date DESC ";
}
